...
| `config_prune_stu_0.8.json` | `false` | 剪枝后学生（由 `apply_prune_s1.py` 生成） |
...
```
即：配置文件路径中不含字符串 `prune` → 加载 safetensors → 使用 `config_tea.json`。
含 `prune` → 跳过 safetensors → 需通过 `--pretrain` 提供权重。
...
| 推理 EER 测试 | `get_embd_w2v.py`（需 checkpoint 同目录有 `train.yaml`） |
